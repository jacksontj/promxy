...
// containsLossyHistogram reports whether v carries any native-histogram
// sample. We treat ANY histogram in an HTTP-pushdown response as lossy
// because the JSON SampleHistogram shape can't preserve sparse-span
...
//
// Post-hoc safety net for queries the AST walker can't pre-flag. When
// triggered, NodeReplacer aborts pushdown and lets the engine evaluate
// locally — Select → GetValue routes through remote_read where the
...
func containsLossyHistogram(v model.Value) bool {
	switch t := v.(type) {
	case model.Vector:
		for _, s := range t {
			if s.Histogram != nil {
